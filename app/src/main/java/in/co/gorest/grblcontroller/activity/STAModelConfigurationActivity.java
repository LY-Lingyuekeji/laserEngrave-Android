...
public class STAModelConfigurationActivity extends AppCompatActivity {

    // 用于日志记录的标签
    private final static String TAG = STAModelConfigurationActivity.class.getSimpleName();
    // 返回
    private ImageView ivBack;
    // 直接连接
    private TextView tvStaModelConfigurationConnect;
    // 请求位置权限的请求码
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 绑定视图
        DataBindingUtil.setContentView(this, R.layout.activity_sta_model_configuration);
    ...
    private void initView() {
        ...
        // 直接连接
        tvStaModelConfigurationConnect = findViewById(R.id.tv_sta_model_configuration_connect);
        // Wi-Fi名称（SSID）
    ...
    private void initListeners() {
        ...
        // 直接连接
        tvStaModelConfigurationConnect.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(STAModelConfigurationActivity.this, STAModelDirectConnectedActivity.class);
            ...
            public void onClick(View v) {
                if (etPassword.getText().length() < 8) {
                    ...
                    if (!etSsid.getText().toString().isEmpty() && !etPassword.getText().toString().isEmpty()) {
                        ...
                        if (!isConnected) {
                            BaseDialog.showCustomDialog(STAModelConfigurationActivity.this, "温馨提示",
                                    "检测到未连接设备，请连接和配置！\r\n\r\n是否进行连接？",
                                    "连接", "取消",
                                    v1 -> {
                                        startActivity(new Intent(STAModelConfigurationActivity.this, AddDeviceActivity.class));
                                        finish();
                                    },
                                    v1 -> {
                                        Log.d(TAG, "用户选择取消");
    ...
    public void onServiceMessageEvent(ServiceMessageEvent event) {
        if (!event.getMessage().isEmpty()) {
            ...
            if (message.contains("[MSG:Mode=AP:")) {
                ...
                // 1. 提取 SSDI（注意字段名是 SSDI，不是 SSID）
                Pattern ssdiPattern = Pattern.compile("SSDI=([^:]+)");
                Matcher ssdiMatcher = ssdiPattern.matcher(message);
                if (ssdiMatcher.find()) {
                    String ssdi = ssdiMatcher.group(1);
                    Log.d(TAG, "SSDI = " + ssdi);
                    ...
                    // TODO 第四步 保存配置信息
                    StaModelConfig staModelConfig = new StaModelConfig();
                    staModelConfig.setMode("STA");
                    staModelConfig.setMachineName(ssdi);
                    staModelConfig.setConfigSSID(etSsid.getText().toString().trim());
                    ...
                    // 读取已有记录
                    List<StaModelConfig> list = FileUtil.readStaModelConfigList(file);
                    // 查找并更新已有记录（按名称唯一判断）
                    boolean updated = false;
                    for (int i = 0; i < list.size(); i++) {
                        if (list.get(i).getConfigSSID().equals(staModelConfig.getConfigSSID())) {
                            list.set(i, staModelConfig); // 替换为最新
                            updated = true;
                            break;
                        }
                    }
                    if (!updated) {
                        list.add(staModelConfig); // 不存在则新增
                    }
                    // 保存
                    FileUtil.saveStaModelConfigList(list, file);

                    // TODO 第五步 跳转页面
                    startActivity(new Intent(STAModelConfigurationActivity.this, STAModelAddActivity.class));
                    finish();
