...
public class PotraceJ
{
    ...
    static final int POTRACE_CORNER = 1;
    static final int POTRACE_CURVETO = 2;
    ...
    /// </summary>
    public static boolean curveoptimizing = true;
    /// <summary>
    ...
    static class Path
    {
        ...
        public String sign = "?";
        // curve
        public ArrayList<Point> pt = new ArrayList<Point>();
        ...
        public int[] po;
        public int[] lon = null;
        public ArrayList<Sum> sums = new ArrayList<Sum>();
        public privcurve curve = null;

    }

    static class privcurve
    {
    ...
    static boolean cyclic(double a, double b, double c)
    {
        if (a <= c)
        {
            return (a <= b && b < c);
    ...
    static void ConvertBitmap(Bitmap bitmap)
    {
        ...
        */
        ...
        for (int i=0; i<result.length; i++)
        {
            ...
            //if (((double)SourcePtr[4 * i + 2] + (double)SourcePtr[4 * i + 1] + (double)SourcePtr[4 * i]) < Treshold * 255 * 3)
            if ((((result[i] >> 0 & 0xFF)) + ((result[i] >> 8 & 0xFF)) + ((result[i] >> 16 & 0xFF))) < 0.5 * 255 * 3)
            {
                result[i] = 1;
            }
            else
        ...
        bm = new Bitmap_p(bitmap.getWidth(), bitmap.getHeight());
        bm.data = result;
    }
    /// <summary>
    ...
    static boolean findNext(Bitmap_p bm1, Point point, Point result)
    {
    ...
    static Path bmToPathlist()
    {
        ...
        boolean weiter = findNext(bm1, currentPoint, currentPoint);
        while (weiter)
        {
            ...
            weiter = findNext(bm1, currentPoint, currentPoint);
        }
   ...
   on failure. */
    ...
    static boolean majority(Bitmap_p bm1, int x, int y)
    {
        int i;
        int a;
        int ct;
    ...
    static void tracetoList(ArrayList<ArrayList<Curve>> ListOfPathes)
    {
    ...
    public static void Clear()
    {
        ...
        PotraceJ.bm = null;
        PotraceJ.pathlist.clear();

    }

    public static ArrayList<ArrayList<Curve>> PotraceTrace(Bitmap bitmap)
    {
        Clear();
        ...
        for (int i = 0; i < pathlist.size(); i++)
        {
            ...
            calcLon(pathlist.get(i));
            bestPolygon(pathlist.get(i));
            adjustVertices(pathlist.get(i));

            smooth(pathlist.get(i));
            if (curveoptimizing)
                optiCurve(pathlist.get(i));

        }

        ArrayList<ArrayList<Curve>> rv = new ArrayList<ArrayList<Curve>>();
        tracetoList(rv);
        return rv;
    }
    ...
    public static ArrayList<String> Export2GCode(ArrayList<ArrayList<Curve>> list, int height, double scale, int feedrate, String lOn, String lOff, String skipcmd, float x, float y)
    {
        ...
        // 回到原点
        String line = "";
        line+= "G0 X"  + String.format("%.2f", 1.0 * startX) + " Y" + String.format("%.2f", 1.0 * startY)+"\r\n";
        rv.add(line);

        return rv;
    }

    private static ArrayList<String> GetPathGC(ArrayList<Curve> Curves, int height, String lOn, String lOff, double scale, String skipcmd)
    {
        ArrayList<String> rv = new ArrayList<String>();

        OnPathBegin(Curves, height, lOn, scale, rv, skipcmd);

        for (Curve curve : Curves)
        {
            OnPathSegment(curve, height, scale, rv);
        }

        OnPathEnd(Curves, lOff, scale, rv);

        return rv;
    }

    private static void OnPathSegment(Curve curve, int height, double scale, ArrayList<String> rv)
    {
        ...
        {
            rv.add(String.format("G1 X%s Y%s\r\n", formatnumber(curve.B.X, scale, true), formatnumber(height - 1 - curve.B.Y, scale, false)));
        }
    ...
    private static void OnPathBegin(ArrayList<Curve> Curves, int height, String lOn, double scale, ArrayList<String> rv, String skipcmd)
    {
        if (Curves.size() > 0)
        {
            //fast go to position
            rv.add(String.format("%s X%s Y%s\r\n", skipcmd, formatnumber(Curves.get(0).A.X, scale, true) , formatnumber(height - 1 - Curves.get(0).A.Y, scale, false)));
            //turn on laser
            rv.add(lOn + "\r\n");
        }
    }

    private static void OnPathEnd(ArrayList<Curve> Curves, String lOff, double scale, ArrayList<String> rv)
    {
        //turn off laser
        if (Curves.size() > 0)
        {
    ...
    private static String formatnumber(double number, double scale, boolean isX)
    {
